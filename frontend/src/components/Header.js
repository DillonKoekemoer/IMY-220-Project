        ...
        <header className="bg-gradient-to-r from-steel-dark to-iron-light shadow-forge sticky top-0 z-50 border-b-2 border-forge-orange backdrop-blur-lg animate-slide-down">
            <nav className="flex justify-between items-center py-6 px-8 max-w-6xl mx-auto min-h-[80px]">
                <div className="flex items-center gap-4">
                    <Link to="/home" className="flex items-center gap-2 text-silver text-2xl font-sans font-bold transition-all duration-300 hover:text-forge-yellow hover:scale-105">
                        <img
                            src={LogoImage}
                            alt="CodeForge Logo"
                ...
                <div className="flex items-center gap-8">
                    ...
                    <div
                        className="w-10 h-10 rounded-full overflow-hidden cursor-pointer transition-all duration-300 hover:scale-110 hover:shadow-forge-glow border-2 border-forge-yellow"
                        onClick={handleProfileClick}
                        title="Profile"
                    >
                        {currentUser?.profilePicture ? (
                            currentUser.profilePicture.startsWith('placeholder-') ? (
                                <div
                                    className="w-full h-full flex items-center justify-center text-sm font-bold text-white"
                                    style={{ backgroundColor: `#${currentUser.profilePicture.replace('placeholder-', '')}` }}
                                >
                                    {currentUser?.name ? currentUser.name.split(' ').map(n => n[0]).join('').toUpperCase() : 'U'}
                                </div>
                            ) : (
                                <img
                                    src={currentUser.profilePicture.startsWith('http') ? currentUser.profilePicture : `http://localhost:3001${currentUser.profilePicture}`}
                                    alt="Profile"
                                    className="w-full h-full object-cover"
                                />
                            )
                        ) : (
                            <div className="w-full h-full bg-gradient-to-r from-forge-orange via-forge-red to-forge-yellow text-white flex items-center justify-center font-semibold text-sm">
                                {currentUser?.name ? currentUser.name.split(' ').map(n => n[0]).join('').toUpperCase() : 'U'}
                            </div>
                        )}
                    </div>
